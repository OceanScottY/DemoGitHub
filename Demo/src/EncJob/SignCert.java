...
public class SignCert {
    ...
    public static void main(String[] args) throws Exception {
        ...
        char[] storepass = "123456".toCharArray();// 存放CA证书和被签证书的证书库的访问密码

        char[] cakeypass = "123456".toCharArray();// CA数字证书条目的访问密码

        String alias = "yuhyCA";// CA证书在证书库中的别名，这个CA的证书用来签名其它的证书

        String name = "E:\\Learning\\Java\\jre1.8.0_131\\bin\\yhyCALib";// 存放CA证书和被签证书的证书库的名字

        String newLib = "SignedLib";// 新证书库的名字，如果需要将签名后的证书放入新库，这是新库的名字

        char[] newLibPass = "123456".toCharArray();// 设置新库的访问密码

        String cerFileName = "E:\\Learning\\Java\\jre1.8.0_131\\bin\\childCA.cer";// 被签证书的证书文件名

        String aliasName = "dataowner";// 被签证书在证书库中的alias别名

        char[] namePass = "123456".toCharArray();// 被签证书的条目在证书库的私钥密码

        int n = 3; // 被签证书的有效期，以年为单位，以当前时间开始计算

        int sn = 201612001;// 序列号可自己定义，这里定义的意义为2004年6月签发，是本年度CA签发的第多少个以001计算，要求唯一

        String afteraliasName = "yhyCASigned_dataowner"; // 签名后新产生的被签过名的证书在库中的别名

        char[] afterNewPass = "123456".toCharArray(); // 签名后新产生的被签过名的证书在库的条目的私钥的密码
        ...
        ///////////////////////////////////////////////////////////////////////////////
         ...
         CertificateFactory.getInstance("X.509");//X.509是使用最多的一种数字证书标准

         FileInputStream in2 = new FileInputStream(cerFileName);//被签证书文件

         java.security.cert.Certificate c2 =
         cf.generateCertificate(in2);//生成需要被签的证书
          in2.close();
...
*/
        ...
        newcert.sign(privateKey, "SHA256WithRSA");// 这样便得到了经过CA签名后的证书
